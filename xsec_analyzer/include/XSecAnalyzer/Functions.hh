...
template <typename T>
T first_or(const std::vector<T>* vp, const T& fallback) {
    return (vp && !vp->empty()) ? (*vp)[0] : fallback;  // or vp->front()
}



inline double finite_or_sentinel(double x, double sentinel = -9999.0) {
    // std::isfinite == false for NaN and ±Inf
    return std::isfinite(x) ? x : sentinel;
}


inline double DistanceWithCheck(double x1, double y1, double z1,
                         double x2, double y2, double z2) {
    // Return -9999 if any input is flagged as invalid
    if (x1 == 99999 || y1 == 99999 || z1 == 99999||
    ...
    // Check for NaN or non-finite result
...
inline double LoC(double vtxx, double vtxy, double vtxz, double pmt1x, double pmt1y, double pmt1z, double pmt2x, double pmt2y, double pmt2z){
