...
{  
 ...
 THStack* Stack = (THStack*)Stack_input->Clone("Stack");
  ...
  if(DoBinWidthNorm==true){
    ...
    ScaleHistogramsInStack(Stack, 1.0/SliceBinWidth, "width" );
    //double ymaxnew = std::max( h_Data->GetMaximum(),
    //h_Total_MC->GetMaximum() ) * 1.5;
    //h_Data->GetYaxis()->SetRangeUser( 0., ymaxnew );
  }
 if (NormalizeAfterBinwith != 1.0){

    h_Data->Scale(NormalizeAfterBinwith);
   h_Total_MC->Scale(NormalizeAfterBinwith);
   scaleTHStack(Stack, NormalizeAfterBinwith);
 ...
 // slice_mc_plus_ext->hist_->GetMaximum() ) * 1.4;
  ...
  //  TH1D *h_upper = (TH1D*)h_Total_MC->Clone("h_upper");
  //  TH1D *h_lower = (TH1D*)h_Total_MC->Clone("h_lower");
 //
  //  for (int i = 1; i <= h_Total_MC->GetNbinsX(); ++i) {
  //      double value = h_Total_MC->GetBinContent(i);
  //      double error = h_Total_MC->GetBinError(i);
 //
  //      h_upper->SetBinContent(i, value + error);
  //      h_lower->SetBinContent(i, value - error);
  //  }
 //
  //  h_upper->SetLineColorAlpha(kBlack, .8);
  //  h_upper->SetLineStyle(2); // Dashed line for upper bound
  //  h_upper->SetLineWidth( 2 );

    //h_lower->SetLineColorAlpha(kBlack, .8);
    //h_lower->SetLineStyle(2); // Dashed line for lower bound
...
// New Style 
////////////////////////////////////////////////////////////////////////////
void DrawStackMCandData_withLineBands(
    TH1* h_data_input, 
    TH1* h_MC_Total_input,
    TH1* BG_beamOFF_input,
    THStack* Stack_input,
    ...
    double YMax)
{
 ...
 THStack* Stack = (THStack*)Stack_input->Clone("Stack");
  ...
  //h_Total_MC->Draw("SAME AXIS");
    ...
    h_upper->SetLineColorAlpha(kBlack, 0.85);
    h_upper->SetLineStyle(3); // Dashed line for upper bound
    h_upper->Draw("HIST SAME"); // Draw the upper bound histogram
    h_upper->SetLineWidth( 1 );
    h_lower->SetLineColorAlpha(kBlack, 0.85);
    h_lower->SetLineStyle(3); // Dashed line for lower bound
    h_lower->Draw("HIST SAME"); // Draw the lower bound histogram
    h_lower->SetLineWidth( 1 );
	...
	double POT_DATA)
{
 ...
 if(setMaxY==true){
  ...
  gc->ResetPads();
	...
	gc->SetYTitleSize(25);
	gc->SetXTitleSize(20); 
	gc->SetYLabel_Size(.04);
	gc->SetXLabel_Size(.04);
	gc->SetYTitle(zaxislabel_units);
...
////////////////////////////////////////////////////////////////////////////////
 ...
 std::map<std::pair<Binning2D, EventCategoryJOINTCC0Pi>, TH1D*> StackMap_input,
 std::map<Binning2D , std::string > BinStringMap,
	char *pdf_label, char *histotitle, char *xaxislabel,
	char *legendTitle, char *zaxislabel_units,
	double Ymax, bool setMaxY, double Ymin,  bool doMultipliers,
...
float BG_Trigger_scaler)
{
 ...
 std::cout<<"inside:PlotMC2D_Stack "<< std::endl;
  ...
  std::vector<EventCategoryJOINTCC0Pi>  Category_vector =  CategoryInterpreter.ReturnCategoryVector();
 ...
 double maxmax = -1; 
    ...
    h_Data_BeamOFF->Scale(BG_Trigger_scaler); 
      ...
      CategoryInterpreter.set_bnb_data_histogram_style( h_Data_BeamOn );
		...
		std::map<EventCategoryJOINTCC0Pi, TH1D*> h_p_EventCategory_StackMap;
       ...
       h_p_EventCategory_StackMap.insert(std::pair<EventCategoryJOINTCC0Pi, TH1D*>(cat,hist)); 
     }
...
void BinWidthNormTHStack(THStack* stack, double scaleFactor) {
    ...
    while ((obj = next())) {
        if (obj->IsA()->InheritsFrom("TH1")) {
            TH1* hist = dynamic_cast<TH1*>(obj);
            hist->Scale(1.0/scaleFactor,"width");
        }
    }
}
