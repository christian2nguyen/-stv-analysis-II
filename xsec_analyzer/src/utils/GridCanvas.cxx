                                  ...
                                  double yoffset)
{
  ...
  for(int i=0; i<fNPadsY; ++i){
    ...
    for(int i=0; i<nLabels; ++i){
      ...
      double xndc= lmarg + (positions[i]-x1)*(1-rmarg-lmarg)/(x2-x1) + f_YTitle_x_offset;
      double yndc= bmarg + (y-y1)*(tmarg-bmarg)/(y2-y1);
...
void GridCanvas::DrawTitles()
{
  ...
  case kAlignCenter:
    ...
    yposx=fBottomMargin + GetSingleFrame_Height() - .27 * GetSingleFrame_Height(); /* height of box (1-fTopMargin-fBottomMargin)*/
