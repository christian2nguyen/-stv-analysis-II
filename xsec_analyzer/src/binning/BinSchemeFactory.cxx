...
#include "XSecAnalyzer/Binning/JOINTCC0Pi_Bin2DCombined.hh"
  ...
  const std::string& bin_scheme_name )
{
  ...
  else if(bin_scheme_name == "cc0picombined"){
    bs = new JOINTCC0Pi_Bin2DCombined;
    bs->Init();
  }    
...
else if(bin_scheme_name == "anniebins"){
    bs = new JOINTCC0Pi_ANNIE;
    bs->Init();
  }
  
  else if(bin_scheme_name == "blips"){
    bs = new JOINTCC0Pi_blips;
    bs->Init();
  }
 else if(bin_scheme_name == "ANNIEDet"){
    bs = new JOINTCC0Pi_ANNIE_det;
    bs->Init();
  }
  ...
  std::cout << "Available binning schemes:\n"
          << "  - CCXp0piBinScheme\n"
