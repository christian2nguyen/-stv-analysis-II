...
//events_ch.SetCacheSize(50 * 1024 * 1024); // Set cache size to 50 MB
 //events_ch.AddBranchToCache("*"); // Add all branches to cache
  ...
  while ( true ) {
    ...
    AnalysisANNIE_Event cur_event;
   if(local_debug_on) std::cout<<"Debug : 1."<< std::endl;
    ...
    if(local_debug_on) std::cout<<"Debug : 2."<< std::endl;
    ...
    if(local_debug_on) std::cout<<"Debug : 3. local_entry = "<<local_entry<< std::endl;

    //Long64_t nEntries = events_ch.GetEntries();
     // if(local_debug_on) std::cout<<"Debug : 3.1 nEntries = "<<nEntries<< std::endl;
    ...
    if(local_debug_on) std::cout<<"Debug : 4."<< std::endl;
    ...
    if(local_debug_on) std::cout<<"Debug : 5."<< std::endl;
    set_event_output_branch_addresses(*out_tree, cur_event, create_them );

   if(local_debug_on) std::cout<<"Debug : 6."<< std::endl;
   ...
   if(local_debug_on) std::cout<<"Debug : 7."<< std::endl;
   ...
   if(local_debug_on) std::cout<<"Debug : 8."<< std::endl;
  for ( auto& sel : selections ) {
    sel->summary();
  }
  if(local_debug_on)std::cout << "Wrote output to:" << output_filename << std::endl;
