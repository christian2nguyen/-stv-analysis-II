...
void Unfolder(std::string XSEC_Config, std::string SLICE_Config, std::string OutputDirectory, std::string OutputFileName) {
  ...
  std::cout<< "Running CrossSectionExtractor"<< std::endl;
  ...
  std::cout<< "Finished: making extr ,  CrossSectionExtractor"<< std::endl;


  // Plot slices of the unfolded result
  std::cout<<"Getting Slice Binning "<< std::endl;
  auto* sb_ptr = new SliceBinning( SLICE_Config );
  auto& sb = *sb_ptr;
  std::cout<<"Finished Slice Binning "<< std::endl;

  std::cout<<"Running get_unfolded_events "<< std::endl;
  auto xsec = extr->get_unfolded_events();
  std::cout<<"Finished get_unfolded_events "<< std::endl;
  std::cout<<"Running conversion_factor "<< std::endl;
  double conv_factor = extr->conversion_factor();
  std::cout<<"Finshed conversion_factor:  "<< conv_factor<< std::endl;
  std::cout<<"Running get_prediction_map "<< std::endl;
  const auto& pred_map = extr->get_prediction_map();
  std::cout<<" Finished  get_prediction_map "<< std::endl;
