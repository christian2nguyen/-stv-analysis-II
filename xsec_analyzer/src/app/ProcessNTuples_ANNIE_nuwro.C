...
#include "XSecAnalyzer/AnalysisEvent_ANNIE_nuwro.hh"
...
bool local_debug_on_nuwro = false; 
  ...
  const std::string& output_filename )
{
  ...
  TChain events_ch( "phaseIITriggerTree" );
  //TChain subruns_ch( "nuselection/SubRun" );

  for ( const auto& f_name : in_file_names ) {
  std::cout<<"Adding f_name = "<< f_name.c_str() << std::endl;
  ...
  out_file->cd();
  TTree* out_tree = new TTree( "phaseIITriggerTree",  "phaseIITriggerTree" );
...
//TTreeCache::SetLearnEntries(10); // Cache for the first 10 entries
//events_ch.SetCacheSize(50 * 1024 * 1024); // Set cache size to 50 MB
 //events_ch.AddBranchToCache("*"); // Add all branches to cache
  ...
  while ( true ) {
    ...
    AnalysisANNIE_Event_nuwro cur_event;
   if(local_debug_on_nuwro) std::cout<<"Debug : 1."<< std::endl;
    ...
    if(local_debug_on_nuwro) std::cout<<"Debug : 2."<< std::endl;
    ...
    if(local_debug_on_nuwro) std::cout<<"Debug : 3. local_entry = "<<local_entry<< std::endl;

    //Long64_t nEntries = events_ch.GetEntries();
     // if(local_debug_on_nuwro) std::cout<<"Debug : 3.1 nEntries = "<<nEntries<< std::endl;
    ...
    if(local_debug_on_nuwro) std::cout<<"Debug : 4."<< std::endl;
    ...
    if(local_debug_on_nuwro) std::cout<<"Debug : 5."<< std::endl;
    set_event_output_branch_addresses(*out_tree, cur_event, create_them );

   if(local_debug_on_nuwro) std::cout<<"Debug : 6."<< std::endl;
   ...
   if(local_debug_on_nuwro) std::cout<<"Debug : 7."<< std::endl;
   ...
   if(local_debug_on_nuwro) std::cout<<"Debug : 8."<< std::endl;
  for ( auto& sel : selections ) {
    sel->summary();
  }
  if(local_debug_on_nuwro)std::cout << "Wrote output to:" << output_filename << std::endl;
...
//out_file->cd();        // Make sure we're in the right directory
//out_file->Write();     // Write everything once
//out_file->Close();
//delete out_file;
  out_tree->Write("", TObject::kOverwrite);  // Force overwrite instead of creating new cycle
  out_file->Close("R");
