...
usage() {
  echo "Usage: $0 -i <input list> -o <out directory> [-v version] [-r runnumbers] [-s samples]"
  echo "  -i file        Specify the input file list to run over."
...
while getopts ":i:o:r:s:v:" opt; do
  case ${opt} in
    i )
      input_file=$OPTARG
      ;;
